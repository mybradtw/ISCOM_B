...
public class BCalendarView extends View {
    ...
    public BCalendarView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i=firstWeek-1; i>=0; i--) {
            canvas.drawText("" + (lastMonthCount-i),
                    (firstWeek-1-i) * cellW + (cellW/2-textSize/2), week1Y, textPaint);
        }
        int day = 1;
        for (int i=firstWeek; i<7; i++){
            canvas.drawText("" + (day++),
                    i * cellW + (cellW/2-textSize/2), week1Y, textPaint);
        }

        // 第一週的內容第一列
        float week1cont1Y = week1Y + textSize + 4;
        canvas.drawText("*",
                0 * cellW + (cellW/2-textSize/2), week1cont1Y, textPaint);

        float line2Y = week1cont1Y + (textSize / 3) + 4;
        ...
        for (int i=0; i<7; i++){
            canvas.drawText("" + (day++),
                    i * cellW + (cellW/2-textSize/2), week2Y, textPaint);
        }

        // 第二週的內容第一列
        float week2cont1Y = week2Y + textSize + 4;
        canvas.drawText("*",
                0 * cellW + (cellW/2-textSize/2), week2cont1Y, textPaint);

        float line3Y = week2cont1Y + (textSize / 3) + 4;
        ...
        for (int i=0; i<7; i++){
            canvas.drawText("" + (day++),
                    i * cellW + (cellW/2-textSize/2), week3Y, textPaint);
        }

        // 第三週的內容第一列
        float week3cont1Y = week3Y + textSize + 4;
        canvas.drawText("*",
                0 * cellW + (cellW/2-textSize/2), week3cont1Y, textPaint);

        float line4Y = week3cont1Y + (textSize / 3) + 4;
        ...
        for (int i=0; i<7; i++){
            canvas.drawText("" + (day++),
                    i * cellW + (cellW/2-textSize/2), week4Y, textPaint);
        }

        // 第四週的內容第一列
        float week4cont1Y = week4Y + textSize + 4;
        canvas.drawText("*",
                0 * cellW + (cellW/2-textSize/2), week4cont1Y, textPaint);

        float line5Y = week4cont1Y + (textSize / 3) + 4;
        ...
        if (day < days){
            ...
            for (int i=0; i<7; i++){
                if (day <= days) {
                    canvas.drawText("" + (day++),
                            i * cellW + (cellW/2-textSize/2), week5Y, textPaint);
                }else{
            ...
            canvas.drawText("*",
                    0 * cellW + (cellW/2-textSize/2), week5cont1Y, textPaint);

            float line6Y = week5cont1Y + (textSize / 3) + 4;
            ...
            if (day < days) {
                ...
                for (int i = 0; i < 7; i++) {
                    if (day <= days) {
                        canvas.drawText("" + (day++),
                                i * cellW + (cellW/2-textSize/2), week6Y, textPaint);
                    } else {
                        canvas.drawText("" + (nextMonthDay++),
                                i * cellW + (cellW/2-textSize/2), week6Y, textPaint);
                    }
                ...
                canvas.drawText("*",
                        0 * cellW + (cellW/2-textSize/2), week6cont1Y, textPaint);

                float line7Y = week6cont1Y + (textSize / 3) + 4;
